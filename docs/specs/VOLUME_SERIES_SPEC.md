...
For any given delivery window and flat volume, the total energy must be identical regardless of the granularity used to decompose it:
...
│   ├── Full materialization tests
│   ├── DST fall-back test
│   ├── DST spring-forward test
│   ├── Partial materialization test
│   ├── Chunk partitioning test
│   └── Formula attachment test
└── CrossCuttingTests
    ├── Energy invariant across granularities
    ├── Interval count scaling
    ├── Bi-temporal timestamp validation
    ├── MONTHLY granularity guard
    └── DeliveryWindow validation
```
...
- Each interval energy: **3.75 MWh**
- Total energy: **15.00 MWh**
...
- Each interval energy: **7.50 MWh**
- Total energy: **22.50 MWh**
...
- Each interval energy: **7.50 MWh**
- Total energy: **15.00 MWh**
- **Cross-granularity invariant**: Total energy equals Scenario 2 (same window, different granularity)
