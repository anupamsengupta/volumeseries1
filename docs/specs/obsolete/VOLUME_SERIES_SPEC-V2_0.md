...
A long-tenor PPA produces a single `VolumeSeries` containing intervals at up to three different granularities. As time progresses, coarse intervals are disaggregated into finer ones (LONG_TERM monthly → MEDIUM_TERM daily → NEAR_TERM base granularity). Non-cascade series (short-term DA/intraday trades) have `cascadeTier = null` on all intervals and a single uniform granularity.
...
Metadata registry for the wide-table scalar columns on `VolumeInterval`. Each entry defines the column's classification, unit, and which cascade tiers it is populated in.

| Column Name | Classification | Unit | Populated In | Description |
|---|---|---|---|---|
| `contractedMw` | `RATE` | MW | All tiers | Contracted capacity from VolumeFormula |
| `planMw` | `RATE` | MW | NEAR_TERM, MEDIUM_TERM | Planned/scheduled volume |
| `actualMw` | `RATE` | MW | NEAR_TERM only | Metered actual delivery |
| `nominatedMw` | `RATE` | MW | NEAR_TERM only | Nominated volume for TSO |
| `basePriceEurMwh` | `RATE` | EUR/MWh | All tiers | Base contract price |
| `greenPremiumEurMwh` | `RATE` | EUR/MWh | All tiers | Green certificate premium |
| `toleranceFloorMw` | `RATE` | MW | All tiers | Min delivery tolerance |
| `toleranceCeilingMw` | `RATE` | MW | All tiers | Max delivery tolerance |
| `forecastMw` | `RATE` | MW | NEAR_TERM, MEDIUM_TERM | Generation forecast (GENERATION_FOLLOWING) |
| `balancingCostEur` | `ABSOLUTE` | EUR | NEAR_TERM only | Balancing market cost for period |
| `imbalancePenaltyEur` | `ABSOLUTE` | EUR | NEAR_TERM only | Imbalance settlement penalty |
| `congestionRentEur` | `ABSOLUTE` | EUR | NEAR_TERM only | Cross-border congestion rent |
| `profileCostEur` | `ABSOLUTE` | EUR | NEAR_TERM, MEDIUM_TERM | Profile shaping cost allocation |
| `customScalars` | N/A | JSONB | All tiers | Tenant-specific dynamic scalars |

Columns not populated in a given tier are stored as `NULL`. TimescaleDB compression eliminates NULL column overhead in compressed chunks.
...
| `totalExpectedIntervals` | `int` | Pre-calculated for progress tracking (sum across all cascade tiers at their effective granularities) |
| `materializedIntervalCount` | `int` | Current count of materialized intervals (across all tiers) |
| `nearTermBoundary` | `ZonedDateTime` | End of M+2 boundary (null for non-cascade short trades) |
| `mediumTermBoundary` | `ZonedDateTime` | End of +1yr from M+2 boundary (null for non-cascade short trades) |
...
- `calculateExpectedIntervals()`: Computes total expected intervals across all cascade tiers at their effective granularities. For non-cascade trades, walks the full delivery timeline at `baseGranularity`. For cascade trades, sums: near-term intervals at `baseGranularity` (deliveryStart → nearTermBoundary) + medium-term intervals at effective mid granularity (nearTermBoundary → mediumTermBoundary) + long-term intervals at effective long granularity (mediumTermBoundary → deliveryEnd). Uses `ZonedDateTime` arithmetic for DST correctness.
...
| `cascadeTier` | `CascadeTier` | Which cascade tier this interval belongs to (null for non-cascade trades) |
...
```
nearTermBoundary = deliveryStart
    .with(TemporalAdjusters.lastDayOfMonth())  // end of current month
    .plusMonths(2)                               // M+2
    .plusDays(1)                                  // start of M+3
    .atStartOfDay(deliveryTimezone)
    
mediumTermBoundary = nearTermBoundary.plusYears(1)

// Clamp to delivery window
nearTermBoundary  = min(nearTermBoundary, deliveryEnd)
mediumTermBoundary = min(mediumTermBoundary, deliveryEnd)
```

If `deliveryEnd` falls within the near-term window, the series has only NEAR_TERM intervals and no cascade. If it falls within the medium-term window, only NEAR_TERM and MEDIUM_TERM tiers exist.

**Generation Flow:**
1. `buildCascadeSeries()` creates a single `VolumeSeries` with:
   - Near-term intervals materialized immediately (FULL for this tier) at `baseGranularity`, each tagged `cascadeTier=NEAR_TERM`
   - `nearTermBoundary` and `mediumTermBoundary` set on the series
   - `materializationStatus = PARTIAL`
   - `materializedThrough` set to the near-term boundary month
...
**Monthly Disaggregation Cron:**

A monthly cron job advances the cascade boundaries:
- `disaggregateMonthlyToDaily(month)`: Removes the LONG_TERM monthly interval for `month`, generates equivalent MEDIUM_TERM daily intervals. RATE scalars are copied; ABSOLUTE scalars are pro-rated by energy weight.
- `disaggregateDailyToBase(fromDate, toDate)`: Removes MEDIUM_TERM daily intervals in `[fromDate, toDate)`, generates equivalent NEAR_TERM base-granularity intervals. Same scalar disaggregation rules apply.
- Both methods update `nearTermBoundary` and `mediumTermBoundary` on the series.
- `totalExpectedIntervals` is recalculated after each disaggregation (finer intervals increase the count).

**Scalar Disaggregation Rules:**

When disaggregating a coarse interval into N finer intervals:

| Classification | Rule | Example |
|---|---|---|
| `RATE` | Copy `parent.value` to every child | Monthly `contractedMw=50` → each daily interval gets `contractedMw=50` |
...
**Energy Invariant:** Disaggregation preserves the energy invariant — the sum of finer-grained energies equals the coarser interval's energy. For ABSOLUTE scalars, the sum of child values equals the parent value (within BigDecimal scale 6 tolerance).

**Result Records:**
- `DisaggregationResult(updatedSeries)`: Returned by disaggregation methods. The series has coarse intervals removed and finer intervals appended, with updated boundaries and counts.
...
**Compression Policy:** Chunks containing intervals whose delivery periods are > 6 months in the past are auto-compressed. The `compress_segmentby` columns are `(tenant_id, trade_id)` to align with the dominant query pattern.

**Retention Policy:** Chunks are auto-dropped when their delivery periods exceed `settlement_finalized_at + 7 years + 6 months` (buffer for dispute resolution delays). The purge job uses the trade's `settlement_finalized_at` timestamp, not the raw delivery date.
...
### 6.8 Scalar Disaggregation Invariant

When disaggregating a parent interval into N child intervals:

**RATE scalars:** Each child gets the parent's value verbatim.

```
∀ child: child.rateScalar == parent.rateScalar
```

**ABSOLUTE scalars:** Children's values sum to parent's value (within BigDecimal scale 6 tolerance).

```
sum(child.absoluteScalar for all children) ≈ parent.absoluteScalar
```

Each child's share is proportional to its energy weight:

```
child.absoluteScalar = parent.absoluteScalar × (child.energy / parent.energy)
```

This invariant is critical for financial reconciliation — the total cost allocated across fine-grained intervals must equal the original coarse-interval cost.
...
└── CascadeMaterializationTests
    ├── buildCascadeSeries produces single series with three tiers of intervals
    ├── Near-term intervals have cascadeTier=NEAR_TERM and effectiveGranularity=baseGranularity
    ├── nearTermBoundary and mediumTermBoundary are correctly computed
    ├── materializeMediumTermChunk generates daily intervals tagged MEDIUM_TERM
    ├── materializeLongTermChunk generates a monthly interval tagged LONG_TERM
    ├── Cross-tier boundary alignment (nearTerm.last.end == mediumTerm.first.start)
    ├── disaggregateMonthlyToDaily moves monthly → daily intervals within same series (energy invariant)
    ├── disaggregateMonthlyToDaily copies RATE scalars, pro-rates ABSOLUTE scalars by energy weight
    ├── disaggregateDailyToBase moves daily → 15-min intervals within same series (energy invariant)
    ├── disaggregateDailyToBase copies RATE scalars, pro-rates ABSOLUTE scalars by energy weight
    ├── ABSOLUTE scalar sum after disaggregation equals parent value (within scale 6 tolerance)
    ├── Daily intervals on DST days have correct energy (25h fall-back, 23h spring-forward)
    ├── DST day ABSOLUTE scalar pro-ration uses actual energy weights (not uniform 1/N)
    ...
    └── Series promotes to FULL when all chunks across all tiers are materialized
```
...
- `buildCascadeSeries(tradeId, tradeLegId, deliveryStart, deliveryEnd, baseGranularity, volume, profileType, volumeUnit, zoneId)`: Creates a **single `VolumeSeries`** with a three-tier cascade (Section 4.5). Computes `nearTermBoundary` (end of M+2) and `mediumTermBoundary` (+1yr from nearTermBoundary). Near-term intervals are materialized immediately at `baseGranularity` with `cascadeTier=NEAR_TERM` and `effectiveGranularity=baseGranularity`. Medium-term and long-term intervals start unmaterialized. RATE scalar columns (`contractedMw`, `basePriceEurMwh`, etc.) are populated from `VolumeFormula` on near-term intervals. Returns a single `VolumeSeries` with `materializationStatus=PARTIAL`.
- `materializeMediumTermChunk(series, chunkMonth, volume)`: Generates daily intervals for the medium-term portion of a cascade series chunk. Each interval is tagged `cascadeTier=MEDIUM_TERM`, `effectiveGranularity=DAILY`. Returns updated series with intervals appended. Promotes to FULL when all expected intervals across all tiers are materialized.
- `materializeLongTermChunk(series, chunkMonth, volume)`: Generates a single monthly interval for the long-term portion of a cascade series chunk. Tagged `cascadeTier=LONG_TERM`, `effectiveGranularity=MONTHLY`. Returns updated series. Promotes to FULL when all expected intervals are materialized.
- `disaggregateMonthlyToDaily(series, month, volume)`: Removes the LONG_TERM monthly interval for `month` from the series and generates equivalent MEDIUM_TERM daily intervals within the same series. RATE scalars are copied verbatim; ABSOLUTE scalars are pro-rated by energy weight (Section 6.8). Updates `mediumTermBoundary`, recalculates `totalExpectedIntervals`. Returns `DisaggregationResult(updatedSeries)`.
- `disaggregateDailyToBase(series, fromDate, toDate, volume)`: Removes MEDIUM_TERM daily intervals in `[fromDate, toDate)` from the series and generates equivalent NEAR_TERM base-granularity intervals within the same series. Same scalar disaggregation rules. Updates `nearTermBoundary`, recalculates `totalExpectedIntervals`. Returns `DisaggregationResult(updatedSeries)`.
