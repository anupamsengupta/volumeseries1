...
CREATE TABLE volume_series.volume_interval (
    ...
    superseded_at               TIMESTAMPTZ,  -- NULL = current version; non-NULL = superseded
    superseded_by_id            BIGINT,       -- ID of the row that supersedes this one
    ...
    CONSTRAINT chk_vi_supersession CHECK (
        -- If superseded_at is set, superseded_by_id must also be set, and vice versa
...
-- Append-only ACTUAL: latest version lookup (BAV materialization, position calc)
-- WHERE clause keeps the index small (current versions only)
CREATE INDEX idx_vi_current_version
    ON volume_series.volume_interval (tenant_id, series_id, interval_start, bucket_type)
...
Two database-level invariants are enforced via triggers: CONTRACTUAL is fully immutable, and ACTUAL is append-only (corrections create new version rows, never updating existing rows).

**ACTUAL append-only via versioning:**

When new ACTUAL data arrives for a delivery interval that already has an ACTUAL row:
1. The application inserts a new row with `version = previous.version + 1` and `superseded_at = NULL`
2. A trigger updates the previous row's `superseded_at = now()` and `superseded_by_id = new.id`
3. The previous row's volume/scalar columns are never modified — only the supersession metadata

This means corrections never require decompressing chunks. New rows always land in the current (hot, uncompressed) chunk regardless of when the corrected delivery period was. The previous version stays in its compressed chunk untouched.

```sql
-- V010__immutability_and_versioning_triggers.sql

-- Trigger for CONTRACTUAL immutability (unchanged from V1.1)
...
-- Trigger for ACTUAL append-only: prevent updates to volume/scalar columns
CREATE OR REPLACE FUNCTION volume_series.prevent_actual_update()
RETURNS TRIGGER AS $$
BEGIN
    IF OLD.bucket_type = 'ACTUAL' AND TG_OP = 'UPDATE' THEN
        -- Allow only supersession metadata updates and settlement_finalized_at
        IF OLD.volume IS DISTINCT FROM NEW.volume
           OR OLD.energy IS DISTINCT FROM NEW.energy
           ...
           OR OLD.interval_end IS DISTINCT FROM NEW.interval_end
        THEN
            RAISE EXCEPTION 'ACTUAL intervals are append-only; corrections must create a new version row (interval id=%)', OLD.id
                USING ERRCODE = '23514';
        END IF;
    END IF;
    RETURN NEW;
...
public void recordActualCorrection(UUID seriesId, ZonedDateTime intervalStart, 
    ...
    previous.setSupersededById(newVersion.getId());
    repository.save(previous);
    // The trigger allows this update because only supersession metadata is changing
}
```

This pattern means:
- New corrections always land in the current (hot, uncompressed) chunk
- Old versions stay in their compressed chunks, untouched
- No decompression workflow required
- Full audit trail preserved automatically
- BAV materialization (Section 11) reads only `WHERE superseded_at IS NULL`
...
### 6.4 Corrections: Append-Only, Not Decompress-Update

When a settled ACTUAL needs correction (TSO submits revised metering data weeks later), the data layer does **not** decompress old chunks. Instead, corrections create new version rows in the current (hot) chunk.

**The correction flow:**

1. Application receives corrected metering data for a past delivery period
2. Application looks up the current version of the affected ACTUAL row (via `idx_vi_current_version` index, fast lookup)
3. Application inserts a new row with `version = previous.version + 1`, the corrected values, and `superseded_at = NULL` — this row lands in the current hot chunk regardless of when the original delivery occurred
4. Application updates the previous row's `superseded_at` and `superseded_by_id` (allowed by the append-only trigger; only metadata fields change)
5. Application triggers BAV materialization refresh for the affected interval (incremental, not full)
6. If the previous row was in a compressed chunk: a single metadata update on a compressed row. PostgreSQL handles this by promoting the row to an uncompressed segment within the chunk (TimescaleDB internal). This is fast and bounded.
...
- BAV materialization (Section 11) reads `WHERE superseded_at IS NULL` to see only current versions
- Audit queries that need historical versions can join on `superseded_by_id` to walk the version chain
...
Some TSOs deliver corrections in monthly batches. The application processes them with a batch size that fits comfortably in a single transaction (typically 1,000–5,000 rows per batch), all hitting the current hot chunk. This is fast and safe.
...
SELECT 
    ...
    -- Pick the single highest-priority bucket's row (where superseded_at IS NULL)
    -- using last() with priority-inverted sort
    last(volume, volume_series.bucket_priority(bucket_type)) FILTER (WHERE superseded_at IS NULL) AS bav_volume,
    last(energy, volume_series.bucket_priority(bucket_type)) FILTER (WHERE superseded_at IS NULL) AS bav_energy,
    last(bucket_type, volume_series.bucket_priority(bucket_type)) FILTER (WHERE superseded_at IS NULL) AS resolved_from_bucket,
    last(effective_granularity, volume_series.bucket_priority(bucket_type)) FILTER (WHERE superseded_at IS NULL) AS resolved_granularity,
    
    -- Bucket-specific scalars: take from the bucket they live in (no resolution needed)
    MAX(contracted_mw) FILTER (WHERE bucket_type = 'CONTRACTUAL' AND superseded_at IS NULL) AS contracted_mw,
    MAX(plan_mw) FILTER (WHERE bucket_type = 'PLAN' AND superseded_at IS NULL) AS plan_mw,
    MAX(actual_mw) FILTER (WHERE bucket_type = 'ACTUAL' AND superseded_at IS NULL) AS actual_mw,
    MAX(nominated_mw) FILTER (WHERE bucket_type = 'PLAN' AND superseded_at IS NULL) AS nominated_mw,
    
    -- Cross-bucket scalars: PLAN overrides CONTRACTUAL where present
    COALESCE(
        MAX(base_price_eur_mwh) FILTER (WHERE bucket_type = 'PLAN' AND superseded_at IS NULL),
        MAX(base_price_eur_mwh) FILTER (WHERE bucket_type = 'CONTRACTUAL' AND superseded_at IS NULL)
    ) AS base_price_eur_mwh,
    
    COALESCE(
        MAX(green_premium_eur_mwh) FILTER (WHERE bucket_type = 'PLAN' AND superseded_at IS NULL),
        MAX(green_premium_eur_mwh) FILTER (WHERE bucket_type = 'CONTRACTUAL' AND superseded_at IS NULL)
    ) AS green_premium_eur_mwh,
    
    -- ACTUAL-only scalars: settlement costs realized post-delivery
    MAX(balancing_cost_eur) FILTER (WHERE bucket_type = 'ACTUAL' AND superseded_at IS NULL) AS balancing_cost_eur,
    MAX(imbalance_penalty_eur) FILTER (WHERE bucket_type = 'ACTUAL' AND superseded_at IS NULL) AS imbalance_penalty_eur,
    MAX(congestion_rent_eur) FILTER (WHERE bucket_type = 'ACTUAL' AND superseded_at IS NULL) AS congestion_rent_eur,
    
    -- Mixed: ACTUAL > PLAN
    COALESCE(
        MAX(profile_cost_eur) FILTER (WHERE bucket_type = 'ACTUAL' AND superseded_at IS NULL),
        MAX(profile_cost_eur) FILTER (WHERE bucket_type = 'PLAN' AND superseded_at IS NULL)
    ) AS profile_cost_eur
...
CREATE INDEX idx_bav_tenant_time 
    ON volume_series.bav_interval (tenant_id, bucket_start DESC);

CREATE INDEX idx_bav_series 
    ON volume_series.bav_interval (series_id, bucket_start);
```
...
This means BAV queries hit the materialized rows directly — no ROW_NUMBER, no window function, no filtering across multiple bucket types at query time. A query like "give me BAV for tenant X for delivery month Y" is a simple range scan on `bav_interval` with the `idx_bav_tenant_time` index.
...
| Query | Before (V1.1 view) | After (V1.2 continuous aggregate) |
|---|---|---|
...
The continuous aggregate definition encodes the resolution rules per scalar:

| Scalar | Resolution Rule | Implementation |
|---|---|---|
| `bav_volume` (volume) | ACTUAL > PLAN > CONTRACTUAL | `last(volume, bucket_priority)` |
| `contracted_mw` | CONTRACTUAL only | `MAX(...) FILTER (WHERE bucket_type = 'CONTRACTUAL')` |
| `plan_mw` | PLAN only | `MAX(...) FILTER (WHERE bucket_type = 'PLAN')` |
| `actual_mw` | ACTUAL only | `MAX(...) FILTER (WHERE bucket_type = 'ACTUAL')` |
| `base_price_eur_mwh` | PLAN if present, else CONTRACTUAL | `COALESCE(plan_value, contractual_value)` |
| `balancing_cost_eur`, `imbalance_penalty_eur`, `congestion_rent_eur` | ACTUAL only | `MAX(...) FILTER (WHERE bucket_type = 'ACTUAL')` |
| `profile_cost_eur` | ACTUAL > PLAN | `COALESCE(actual_value, plan_value)` |

`MAX()` is used as the aggregation function because there's only one row per `(bucket_type, superseded_at IS NULL)` combination per delivery interval — so MAX is just "pick the value." It could equivalently be `MIN()` or `bool_or()`.

### 11.7 BAV Is the Sole Source for Position

Position calculations always read from `bav_interval`, never from `volume_interval` directly. This guarantees:
...
Net position is not a separately maintained table — it is a **continuous aggregate derived from `bav_interval`**. The composition makes the relationship explicit: position = aggregation of BAV across all series for the same delivery context.

```sql
-- V016__net_position_continuous_aggregate.sql
...
SELECT 
    ...
    -- Net delivered volume = sum of (signed BAV across all trades)
    -- Direction sign comes from the trade leg (BUY positive, SELL negative)
    SUM(
        CASE vs.trade_direction
            WHEN 'BUY' THEN COALESCE(bav.bav_volume, 0)
            WHEN 'SELL' THEN -COALESCE(bav.bav_volume, 0)
        END
    ) AS net_volume_mw,
    
    SUM(
        CASE vs.trade_direction
            WHEN 'BUY' THEN COALESCE(bav.bav_energy, 0)
            WHEN 'SELL' THEN -COALESCE(bav.bav_energy, 0)
        END
    ) AS net_energy_mwh,
    
    -- Realized cost components: sum across all trades
    ...
    SUM(COALESCE(bav.profile_cost_eur, 0)) AS total_profile_cost_eur,
